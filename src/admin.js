...
window.Admin = {
  ...
  showUserProfile(userId) {
    ...
    const renderModal = () => {
      ...
      var reasonsByDate = {}
      Object.keys(userLogs).forEach(function (key) {
        var log = userLogs[key]
        var logDate = log.date || (log.timestamp ? log.timestamp.split('T')[0] : '')
        if (logDate && log.reason && log.reason !== 'تم الحفظ التلقائي لليوم') {
          reasonsByDate[logDate] = log.reason
        }
      })
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="stats-timeline" id="stats-timeline-${userId}">
            ${Admin._renderTimeline(Admin._activeStatTab, penaltyDates, bonusDates, zeroDates, userPoints, evalCategoryMap, reasonsByDate)}
          </div>
  ...
  _renderTimeline(activeTab, penaltyDates, bonusDates, zeroDates, userPoints, evalCategoryMap, reasonsByDate) {
    ...
    if (activeTab === 'bonus' && bonusDates.length > 0) {
      return bonusDates.map(function (d) {
        ...
        <div class="timeline-item">
          ...
          <div class="timeline-content">
            <div class="timeline-date">${d}</div>
            <div class="timeline-desc" style="color:var(--green)">${parts.join(' ، ')}</div>
            ${reasonHtml}
          </div>
        </div>`
      }).join('')
